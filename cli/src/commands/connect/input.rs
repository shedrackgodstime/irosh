...
pub struct InputEngine {
    ...
    /// Whether we are currently swallowing an ANSI sequence from the local terminal.
    swallow_ansi: bool,
}
...
impl InputEngine {
    ...
    fn handle_remote_byte(&mut self, byte: u8, to_remote: &mut Vec<u8>) {
        ...
        // If we are parsing an ANSI sequence generated by the local terminal 
        // (like a Cursor Position Report reply to ConPTY), we must NOT treat it 
