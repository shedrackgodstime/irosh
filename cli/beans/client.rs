...
async fn main() -> Result<()> {
    ...
    if let Some(password) = args.auth_password {
        let user = args.auth_user.unwrap_or_else(|| "admin".to_string());
    ...
    for f in &args.forward {
            ...
            .await
        {
            ...
            Err(e) => {
                eprintln!("Failed to start local forward for {}: {}", f, e);
            }
        }
    }

    for f in &args.remote_forward {
        tracing::info!("Setting up remote tunnel: {}", f);
        let parts: Vec<&str> = f.split(':').collect();
        if parts.len() != 3 {
            eprintln!(
                "Invalid remote forward specification: {}. Use REMOTE_PORT:LOCAL_HOST:LOCAL_PORT",
                f
            );
            continue;
        }
            ...
            .await
        {
            ...
            Err(e) => {
                eprintln!("Failed to start remote forward for {}: {}", f, e);
            }
        }
    }
