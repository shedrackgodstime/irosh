# Irosh Feature Proposals

This document contains speculative, experimental, and next-generation feature proposals for Irosh. These concepts are under discussion and may be updated, implemented, or discarded during future project assessments.
...
*   **SSHFS & Remote Directory Mounting**: Enable users to mount a remote peer's directory directly onto their local filesystem over the secure P2P tunnel:
    *   *Proxy Mode*: Allow standard `sshfs` clients to run out-of-the-box by routing through the **Local SSH Proxy** gateway.
    *   *Native Mounting (`irosh mount`)*: Implement a native command (e.g., `irosh mount <peer-name>:<remote-path> <local-path>`) utilizing FUSE bindings (like `fuser` in Rust) for a seamless, dependency-free mounting experience.

### Native SSH Config Integration (ProxyCommand) [Under Consideration]
*   A helper command (e.g., `irosh config integrate-ssh`) to automatically update or append rules to the user's local SSH config (`~/.ssh/config`).
*   Configures a `ProxyCommand` directive using Irosh as the tunnel gateway, enabling developers to use native OpenSSH commands (`ssh <peer-name>`), scripts, and standard tooling directly with P2P targets.
...
*   A static, WebAssembly-based terminal client running entirely in the web browser.
*   Enables secure P2P connection to remote Irosh nodes from any device (e.g., Chromebooks, tablets, or locked-down office computers) without installing the client binary.

### Standalone "Magic Send" Subcommands (`irosh send` / `irosh recv`)
*   Zero-config, top-level subcommands for transferring files or directories directly over P2P without establishing an interactive shell session (similar to `magic-wormhole` or `croc`).
*   `irosh send <file-path>` generates a short, ephemeral wormhole pairing code, and `irosh recv <code>` receives the payload securely on the remote end.

### Peer-to-Peer Folder Syncing (`irosh sync`)
*   Continuous directory synchronization between two remote Irosh nodes (e.g., backup folders, configurations, or logs) over the P2P network.
*   Enables secure folder syncing across firewalls and NATs without exposing ports or requiring intermediate cloud storage buckets.

### Interactive "Trust Request" Protocol
*   Allow client nodes to request trust and connection authorization interactively, eliminating manual key exchange.
*   A client attempting to connect triggers a real-time console prompt on the host machine (`Peer 'work-laptop' is requesting shell access. Approve? [y/N]`). Upon approval, the nodes exchange and authorize public keys automatically.

### Private Relay Self-Hosting
*   A utility (e.g., via `irosh system deploy-relay` or a container-packaged build) to deploy a private, self-hosted Iroh relay (DERP) node on cloud providers (like Fly.io, DigitalOcean, or AWS).
*   Guarantees 100% data sovereignty and predictable bandwidth/latency by removing dependence on the default public relay infrastructure.
